...
void _handle_udp_coap_message(udp_microcoap_wrapper *obj) {
    ...
    if ((sz = udp->parsePacket()) > 0) {
        ...
        if (obj->b_debug) {
            Serial.print("udp_microcoap_wrapper rx UDP(len="); Serial.print(sz); Serial.print(")> ");
            /*for (i = 0; i < sz; i++) {
        ...
        if (0 != (rc = coap_parse(&pkt, obj->packetbuf, sz))) {
            if (obj->b_debug) {
                Serial.print("udp_microcoap_wrapper> Bad packet rc=");
                Serial.println(rc, DEC);
            }
        } else {
            if (obj->b_debug) {
                Serial.println("udp_microcoap_wrapper> Processing CoAP message");
            }
            ...
            if (0 != (rc = coap_build(obj->packetbuf, &rsplen, &rsppkt))) {
                if (obj->b_debug) {
                    Serial.print("udp_microcoap_wrapper> coap_build failed rc=");
                    Serial.println(rc, DEC);
                }
            } else {
                if (obj->b_debug) {
                    Serial.print("udp_microcoap_wrapper tx UDP(len="); Serial.print(rsplen); Serial.print(")> ");
                    /*for (i = 0; i < rsplen; i++) {
                        Serial.print(obj->packetbuf[i], HEX);
                        Serial.print(" ");
                    }*/
